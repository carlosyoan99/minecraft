...
// 1) Decaimiento: la saturación se consume primero (30s parado -> -1 saturación)
{
  ...
  check('decaimiento 30s parado -> sat 19, food 20',
    p.saturation === 19 && p.food === 20, `sat=${p.saturation} food=${p.food}`);
}
...
// 2) Decaimiento acelerado en movimiento: 15s moviéndose -> -1 saturación
{
  ...
  check('decaimiento 15s en movimiento -> sat 19',
    p.saturation === 19 && p.food === 20, `sat=${p.saturation} food=${p.food}`);
}
...
// 3) Con la saturación agotada, el decaimiento consume comida
{
  ...
  players.tickPlayer(p, 30000);
  check('decaimiento con sat 0 -> food 19',
    p.food === 19 && p.saturation === 0, `food=${p.food} sat=${p.saturation}`);
}
...
// 7) Inanición con health 1 -> muere y respawnea (salud, comida y saturación reset)
{
  ...
  check('muerte por inanicion -> respawn health 20', p.health === 20, `health=${p.health}`);
...
// 8) El decaimiento no baja de 0
{
  ...
  players.tickPlayer(p, 30000);
  check('decaimiento no baja de 0', p.food === 0 && p.saturation === 0,
