...
const timer = setTimeout(() => {
  ...
  if (phase === 'hunt' && results.every((r) => r.ok !== false)) {
    console.log(`[t=${t}s] SKIP: no apareció un pasivo a <4 bloques — bonus de comer omitido (cubierto por los tests unitarios)`);
    finish(0);
  } else {
...
ws.on('message', (d) => {
  ...
  if (phase === 'init' && m.event === 'init') {
    ...
    check('init saturation=20 (wire incluye saturación)', m.data.saturation === 20, 'sat=' + m.data.saturation);
  ...
  if (phase === 'decay' && m.event === 'health_update') {
    damaged = true;
    console.log(`[t=${t}s] daño recibido (health=${m.data.health}) — esperando el food_update de regeneración...`);
    return;
  }
  ...
  if (phase === 'decay' && m.event === 'food_update') {
    if (damaged) {
      ...
      check('anomalía explicada: la regeneración consume comida (food 19, sat 20)', okRegen,
        `t=${t}s food=${m.data.food} sat=${m.data.saturation}`);
    } else {
      const okDecay = m.data.food === 20 && m.data.saturation === 19;
      check('el decaimiento consume saturación primero (food 20, sat 19)', okDecay,
        `t=${t}s food=${m.data.food} sat=${m.data.saturation}`);
    }
    lastFood = m.data.food; lastSat = m.data.saturation;
    phase = 'hunt';
  ...
  if (phase === 'hunt' && m.event === 'mobs_update') {
    const near = m.data.find((mo) =>
      PASSIVE.has(mo.type) &&
      Math.hypot(mo.x - 0.5, mo.y - spawnY, mo.z - 0.5) < 4);
    if (near && hitsSent < 8) {
      ws.send(JSON.stringify({ event: 'attack_mob', data: { mobId: near.id } }));
      hitsSent++;
    }
    return;
  }
  ...
  if (phase === 'hunt' && m.event === 'inventory_update') {
    const foodItem = m.data.inventory.find((s) => s && s.id >= 107 && s.id <= 110);
    if (!foodItem) return;
    check('drop de comida cruda al matar pasivo (bonus)', true, 'item=' + foodItem.id + ' x' + foodItem.count);
    const slotIdx = m.data.inventory.findIndex((s) => s && s.id === foodItem.id);
    if (slotIdx !== 0) ws.send(JSON.stringify({ event: 'inventory_select', data: { slot: slotIdx } }));
  ...
  if (phase === 'eat' && m.event === 'food_update') {
    ...
    check('comer restaura hambre/saturación (bonus)', improved,
