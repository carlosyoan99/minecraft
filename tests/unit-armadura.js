...
const {
  B, I, ARMOR_SLOTS, ARMOR_DURABILITY, isArmor
...
// ============================================================
{
  ...
  for (const [mat, mid] of Object.entries(materials)) {
    for (const [piece, shape] of Object.entries(shapes)) {
      ...
      const expected = base[mat] + ["helmet", "chestplate", "leggings", "boots"].indexOf(piece);
      if (r && r.result.id === expected) allOk++;
      else console.log(`FAIL: receta ${mat}_${piece} → ${r ? r.result.id : "sin receta"} (esperado ${expected})`);
    }
  }
  check(`las 12 recetas de armadura (${allOk}/12)`, allOk === 12);
  check("el cuero NO es crafteable (se obtiene de la vaca/conejo)", !crafting.matchRecipe([{ id: 132, count: 1 }, null, null, null, null, null, null, null, null]));
}
...
ctx.ws.emit("message", JSON.stringify({ event: "equip_armor", data: { inventorySlot: helmSlot } }));
check("equipar el casco → armor.helmet asignado", p.armor.helmet && p.armor.helmet.id === I.IRON_HELMET);
check("equipar el casco → slot del inventario vacío", !p.inventory[helmSlot]);
check("equipar crea la pieza con durabilidad plena", p.armor.helmet.durability === ARMOR_DURABILITY[I.IRON_HELMET], p.armor.helmet.durability + "");
check("inventory_update incluye la armadura", ctx.ws.events("inventory_update").some((m) => m.data.armor && m.data.armor.helmet));
...
ctx.ws.emit("message", JSON.stringify({ event: "equip_armor", data: { inventorySlot: helmSlot2 } }));
check("equipar otro casco → el previo vuelve con su durabilidad (100)", p.inventory.some((s) => s && s.id === I.IRON_HELMET && s.durability === 100), JSON.stringify(p.inventory.filter((s) => s && s.id === I.IRON_HELMET)));
check("el casco nuevo queda con durabilidad plena", p.armor.helmet.durability === ARMOR_DURABILITY[I.IRON_HELMET]);

// Desequipar: vuelve al inventario con su durabilidad
ctx.ws.sent.length = 0;
ctx.ws.emit("message", JSON.stringify({ event: "unequip_armor", data: { slot: "helmet" } }));
...
ctx.ws.emit("message", JSON.stringify({ event: "equip_armor", data: { inventorySlot: 0 } }));
check("equipar un no-armadura → ignorado", !p.armor.helmet && p.inventory[0].id === I.STICK);
...
// ============================================================
{
  ...
  ws.emit("message", JSON.stringify({ event: "equip_armor", data: { inventorySlot: slot } }));
  p2.health = 20;
  playerHelpers.damagePlayer(p2, 10); // pechera hierro: -12% → 8.8 → redondea a 9
  check("daño 10 con pechera de hierro → recibe 9", p2.health === 11, `health=${p2.health}`);
  check("la pechera se desgasta (-2 por 10 de daño bruto)", p2.armor.chestplate.durability === ARMOR_DURABILITY[I.IRON_CHESTPLATE] - 2, p2.armor.chestplate.durability + "");
}
...
// ============================================================
{
  ...
  check("dos botas de cuero → 2 slots separados (no se apilan)", boots.length === 2, `slots=${boots.length}`);
  check("cada pieza conserva su durabilidad (10 y plena)", boots.some((s) => s.durability === 10) && boots.some((s) => s.durability === ARMOR_DURABILITY[I.LEATHER_BOOTS]));
}
...
// ============================================================
{
  ...
  const slot = p4.inventory.findIndex((s) => s && s.id === I.DIAMOND_CHESTPLATE);
  ws.emit("message", JSON.stringify({ event: "equip_armor", data: { inventorySlot: slot } }));
  p4.food = 0;
  p4.health = 10;
  playerHelpers.tickPlayer(p4, 2000); // 1 tick de inanición
  check("inanición daña sin desgastar la armadura", p4.armor.chestplate.durability === ARMOR_DURABILITY[I.DIAMOND_CHESTPLATE], `dur=${p4.armor.chestplate.durability}`);
  check("la inanición sigue restando salud", p4.health === 9, `health=${p4.health}`);
}
...
// ============================================================
{
  ...
  net.handleConnection(ws);
  const init = ws.events("init")[0];
  check("el init incluye armor", !!init.data.armor && init.data.armor.helmet === null && ARMOR_SLOTS.every((s) => s in init.data.armor), JSON.stringify(Object.keys(init.data.armor || {})));
}
