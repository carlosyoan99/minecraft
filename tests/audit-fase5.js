...
const { TOOL_DURABILITY, XP_PER_LEVEL, MAX_LEVEL_HEALTH_BONUS, B, I } = require(path.join(ROOT, 'constants.js'));
...
// ============================================================
console.log('========== PARTE 1: SINCRONIZACIÓN SERVIDOR ↔ CLIENTE ==========\n');
...
// 1a) TOOL_DURABILITY (servidor) == DURABILITY (cliente): parse del ESM
{
  ...
  if (m) {
    ...
    const client = Object.fromEntries(entries.map((e) => [Number(e[1]), Number(e[2])]));
    ...
    check(`DURABILITY cliente == TOOL_DURABILITY servidor (${entries.length} herramientas)`, equal);
    check('XP_PER_LEVEL cliente == servidor (100)', /export const XP_PER_LEVEL = 100;/.test(src));
  }
}
...
{
  const sent = [];
  const p = mkPlayer({ ws: { readyState: OPEN, send: (s) => sent.push(JSON.parse(s)) } });
  ...
  check('inventory_update incluye durability en el wire', slot && slot.durability === 1234,
    JSON.stringify(slot));
}
...
// 1c) El HUD pinta la barra de durabilidad (public/ui.js usa DURABILITY)
{
  ...
  check('ui.js usa item.durability para el ancho', /item\.durability/.test(src));
  check('el servidor avisa tool_broke al romperse (net.js)', /tool_broke/.test(fs.readFileSync(path.join(ROOT, 'net.js'), 'utf8')));
}
...
// piedra con un pico de durabilidad 1): setBlock(AIR) → drop → wear.
{
  ...
  check('drop añadido UNA vez (1 adoquín)', playersMod.countInInventory(p, B.COBBLESTONE) === 1,
    'adoquín=' + playersMod.countInInventory(p, B.COBBLESTONE));
  check('herramienta rota SIN copias (0 picos)', playersMod.countInInventory(p, I.STONE_PICKAXE) === 0,
    'picos=' + playersMod.countInInventory(p, I.STONE_PICKAXE));
  check('total de slots coherente (1 drop, 0 herramientas)', p.inventory.filter(Boolean).length === 1);
}
...
// Romper con la mano (sin herramienta): no desgasta nada y el drop aparece
{
  ...
  check('romper a mano: sin desgaste y 1 drop', p.inventory.filter(Boolean).length === 1 &&
...
// Varios usos seguidos con la misma herramienta: nunca aparecen copias fantasma
{
  ...
  check('6 drops exactos (sin duplicar)', playersMod.countInInventory(p, B.COBBLESTONE) === 6);
  check('herramienta desapareció (0 copias)', playersMod.countInInventory(p, I.IRON_PICKAXE) === 0);
}
...
}
{
  ...
  check('nivel 15 pero maxHealth tope en 30 (+10)', p.level === 15 && p.maxHealth === 20 + MAX_LEVEL_HEALTH_BONUS,
...
}
{
  ...
  check('respawn usa maxHealth (24) y conserva nivel 4', p.health === 24 && p.level === 4,
...
}
{
  ...
  check(`applyToolWear barato (10k usos en ${ms.toFixed(1)} ms)`, ms < 1000, ms.toFixed(1) + 'ms');
}
...
console.log('\n========== PARTE 4: REGRESIÓN FASE 3/4 ==========\n');
{
  ...
  check('vaca sigue dando carne (Fase 3 intacta)', cow && cow[0] && cow[0].id === I.BEEF, JSON.stringify(cow));
}
check('isSolidBlock(SNOW) intacto (Fase 4)', require(path.join(ROOT, 'constants.js')).isSolidBlock(21) === true);
check('world.getHeight intacto (Fase 4)', typeof world.getHeight === 'function');
