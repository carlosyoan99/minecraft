...
check(
	...
	breakSeconds(I.IRON_PICKAXE, B.STONE) <
...
check(
	...
	canHarvest(I.WOODEN_AXE, B.IRON_ORE) === false
);
...
// --- 4) Cancelaciones ---
{
	...
	check(
		"break_cancel devuelve true y limpia la sesión",
		mining.cancelMining(p, sendFn) === true && p.mining === null
	);
	check(
		"break_cancel avisa con stage -1",
		sends.some((s) => s.ev === "block_break_progress" && s.data.stage === -1)
	);
	check(
		"cancelar sin sesión devuelve false",
		mining.cancelMining(p, sendFn) === false
	);
}
{
	const p = mkPlayer();
	...
	check(
		"bloque cambiado → mina cancelada (cancelled)",
		r === "cancelled" && p.mining === null
	);
}
{
	...
	check(
		"mina activa con el jugador cerca",
		mining.tickMining(p, 20, world, playerHelpers, sendFn) === null &&
			p.mining !== null
	);
	p.x = 30; // se aleja > 7 bloques
	const r = mining.tickMining(p, 50, world, playerHelpers, sendFn);
	check(
		"alejarse del bloque → mina cancelada",
		r === "cancelled" && p.mining === null
	);
}
...
// drop, sin XP y sin desgaste; tickMining completa al instante ---
{
	...
	const r = mining.tickMining(p, 50, world, playerHelpers, sendFn);
	check(
		"creative: la sesión activa se completa al instante (done)",
		r === "done" && p.mining === null
	);
	check(
...
// --- 5) Herramienta equivocada / a mano: lento y sin drop (pero rompe) ---
{
	...
	for (let i = 0; i < 400 && !result; i++)
		result = mining.tickMining(p, 20, world, playerHelpers, sendFn);
	check(
		"herramienta equivocada: rompe igual (lento)",
		result === "done" && world.getBlock(8, 5, 5) === B.AIR
	);
	check(
		"sin drop (piedra sin pico)",
		!p.inventory.some((s) => s && s.id === B.COBBLESTONE)
	);
	check(
...
}
{
	...
	for (let i = 0; i < 200 && !result; i++)
		result = mining.tickMining(p, 20, world, playerHelpers, sendFn);
	check(
		"a mano rompe tierra y suelta drop",
		result === "done" &&
			world.getBlock(9, 5, 5) === B.AIR &&
			p.inventory.some((s) => s && s.id === B.DIRT)
	);
	check(
		"a mano no desgasta (sin herramienta)",
		p.inventory[0] === null || p.inventory[0].durability === undefined
	);
}
