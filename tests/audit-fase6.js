...
const { B, CHUNK_SIZE, WORLD_HEIGHT } = require(path.join(ROOT, 'constants.js'));
    ...
    for (let y = 0; y < WORLD_HEIGHT; y++) {
      for (let z = 0; z < CHUNK_SIZE; z++) {
        ...
        for (const [dx, dy, dz] of DIRS) {
          if (clientShouldDraw(block, world.getBlock(wx + dx, wy + dy, wz + dz))) faces++;
        }
      }
    }
  }
...
function lodTier(dist) { return dist > 56 ? 'lod' : 'full'; }

console.log('========== PARTE 1: LOD — CARAS Y TRIÁNGULOS ==========\n');
console.log('Generando área de render completa (radio 6 = 13×13 chunks, disco ignorado)...');
...
console.log(`Spawn: (${px.toFixed(1)}, ${pz.toFixed(1)}) — decide el tier de cada chunk`);

let fullFaces = 0, lodFaces = 0, fullChunks = 0, lodChunks = 0, total = 0;
for (let cx = -R; cx <= R; cx++) {
  for (let cz = -R; cz <= R; cz++) {
    total++;
    const cxp = cx * CHUNK_SIZE + CHUNK_SIZE / 2, czp = cz * CHUNK_SIZE + CHUNK_SIZE / 2;
...
let allFullFaces = 0;
for (let cx = -R; cx <= R; cx++) for (let cz = -R; cz <= R; cz++) allFullFaces += countFullFaces(cx, cz);
...
console.log(`Caras: ${fullFaces.toLocaleString()} full + ${lodFaces.toLocaleString()} LOD (quads)`);
console.log(`Triángulos: ${withLodTris.toLocaleString()} CON LOD vs ${noLodTris.toLocaleString()} sin LOD → reducción del ${(reduction * 100).toFixed(1)}%`);
console.log(`Coste medio por chunk: ${(withLodTris / total).toFixed(0)} triángulos con LOD vs ${(noLodTris / total).toFixed(0)} sin LOD`);
console.log('  (referencia: auditoría Fase 2 renderizaba ~310K triángulos estables)');

check('Perf: el LOD reduce los triángulos del área activa ≥ 20% (los lejanos dominan el recuento bruto)', reduction >= 0.2, (reduction * 100).toFixed(1) + '%');
check('Perf: los chunks lejanos (radio 6) son mayoría LOD, como espera el diseño', lodChunks > fullChunks, `${lodChunks} lod vs ${fullChunks} full`);
...
// Un chunk LOD individual debe costar MUCHO menos que su equivalente full.
{
  ...
  let lodF = 0, lodQ = 0, lodCx = 0, lodCz = 0;
  for (let cx = R; cx <= R; cx++) for (let cz = R; cz <= R; cz++) {
    const cxp = cx * CHUNK_SIZE + CHUNK_SIZE / 2, czp = cz * CHUNK_SIZE + CHUNK_SIZE / 2;
    if (Math.hypot(px - cxp, pz - czp) > 56) { lodCx = cx; lodCz = cz; break; }
  }
  ...
  check('Perf: un chunk LOD cuesta < 30% de su versión full (ahorro dominante en el anillo)', lodQ / lodF < 0.3, `${(100 - (lodQ / lodF) * 100).toFixed(1)}% ahorro`);
}
...
const FLOATS_FULL = 8, FLOATS_LOD = 9, BYTES = 4;
...
console.log(`Geometría CON LOD: ${(memWithLod / 1024 / 1024).toFixed(2)} MB (${fullFaces.toLocaleString()} caras full + ${lodFaces.toLocaleString()} quads LOD)`);
console.log(`Geometría sin LOD: ${(memNoLod / 1024 / 1024).toFixed(2)} MB (${allFullFaces.toLocaleString()} caras full)`);
console.log(`Ahorro en memoria GPU/JS: ${(100 - (memWithLod / memNoLod) * 100).toFixed(1)}% (${((memNoLod - memWithLod) / 1024 / 1024).toFixed(2)} MB)`);
check('Mem: el área activa con LOD cabe holgada en presupuesto (< 30 MB de geometría bruta)', memWithLod < 30 * 1024 * 1024, (memWithLod / 1024 / 1024).toFixed(2) + ' MB');
check('Mem: con LOD la geometría bruta se reduce ≥ 40% vs todo full', memWithLod / memNoLod < 0.6, (100 - (memWithLod / memNoLod) * 100).toFixed(1) + '% ahorro');
...
(async () => {
  ...
  const tmp = path.join(require('os').tmpdir(), `audit-geopool-${process.pid}.mjs`);
  require('fs').copyFileSync(src, tmp);
  const { createGeometryPool, setOrReuseAttribute } = await import('file://' + tmp);
  ...
  function makeFakeGeometry() {
    return {
      ...
      setAttribute(n, a) { this.attrs.set(n, a); return this; },
  ...
  let attrAllocs = 0;
  function CountingAttr(data, itemSize) { attrAllocs++; return { array: new Float32Array(data), itemSize, needsUpdate: false }; }

  const pool = createGeometryPool({ makeGeometry: makeFakeGeometry, maxPooled: 24 });
  ...
  setOrReuseAttribute(geo1, 'position', new Float32Array(6000), 3, CountingAttr); // 6000 floats = 2000 caras
  const s1 = pool.stats();
  ...
  setOrReuseAttribute(geo2, 'position', new Float32Array(6000), 3, CountingAttr);
  const s2 = pool.stats();
  check('Pool: cargar un chunk nuevo tras descargar uno reutiliza la geometría (0 creadas nuevas)', s2.created === s1.created && s2.reused === s1.reused + 1, `created ${s1.created}→${s2.created}, reused ${s1.reused}→${s2.reused}`);
  check('Pool: la reutilización devuelve la MISMA geometría (sin alloc ni dispose)', geo2 === geo1 && !geo1.disposed);
  check('Pool: atributo de mismo tamaño reutiliza el array (0 allocs de Float32Array)', attrAllocs === allocs1, `${allocs1}→${attrAllocs} allocs`);
  ...
  check('Pool: el exceso sobre maxPooled (24) se libera con dispose y no crece sin límite', s3.disposed === batch.length - 24 && s3.created === beforeDispose, `${s3.disposed} liberadas de ${batch.length} (retiene 24), created ${beforeDispose}→${s3.created}`);

  // setOrReuseAttribute con tamaño DISTINTO sí crea (1 alloc nuevo).
  const geo3 = pool.acquire('lod');
  const allocs3 = attrAllocs;
  setOrReuseAttribute(geo3, 'position', new Float32Array(12000), 3, CountingAttr);
  check('Pool: atributo de tamaño distinto crea uno nuevo (1 alloc)', attrAllocs === allocs3 + 1, `${allocs3}→${attrAllocs}`);
  console.log(`  Estadísticas finales del pool: ${pool.stats().created} creadas · ${pool.stats().reused} reutilizadas · ${pool.stats().disposed} liberadas`);
})()
.then(() => {
  ...
  check('Gen: la geometría LOD de un chunk regenerado es idéntica (sin costuras)', lodA === lodB, `${lodA} quads en ambas regeneraciones`);

  world.setDiskLoader(null);
  console.log(`\n${fails === 0 ? '✅ AUDITORÍA FASE 6: todos los checks pasan' : `❌ ${fails} checks fallaron`}`);
  process.exit(fails ? 1 : 0);
})
.catch((e) => { console.error('ERROR en auditoría:', e); process.exit(1); });
