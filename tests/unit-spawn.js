...
check('findSpawn(0,0) no cae en un lago', !world.isLake(Math.floor(s.x), Math.floor(s.z)), `(${s.x},${s.z})`);
check('findSpawn(0,0) y = getHeight(columna)+2', s.y === world.getHeight(Math.floor(s.x), Math.floor(s.z)) + 2, 'y=' + s.y);
check('findSpawn(0,0) x/z centrados en la columna (+0.5)', s.x === Math.floor(s.x) + 0.5 && s.z === Math.floor(s.z) + 0.5);
...
for (let dx = -20; dx <= 20; dx++) {
  for (let dz = -20; dz <= 20; dz++) {
    ...
    check(`findSpawn(${dx},${dz}) no cae en un lago`, !world.isLake(Math.floor(p.x), Math.floor(p.z)), `→ (${p.x},${p.z})`);
    check(`findSpawn(${dx},${dz}) y correcto`, p.y === world.getHeight(Math.floor(p.x), Math.floor(p.z)) + 2, 'y=' + p.y);
  }
}
check('hay lagos en el área probada (la premisa del test es real)', lakesHallados > 0, `${lakesHallados} lagos`);

// --- 3) Determinismo: misma entrada → misma salida ---
const a = world.findSpawn(3, -7), b = world.findSpawn(3, -7);
...
check('columna firme → se usa tal cual', Math.floor(f.x) === firme.x && Math.floor(f.z) === firme.z);
