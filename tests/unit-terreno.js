...
let minaCols = 0, aboveSurface = 0, inBedrock = 0;
...
check("hay columnas con mina en el área", minaCols > 50, `${minaCols} columnas`);
check("los túneles de mina nunca rompen la superficie", aboveSurface === 0, `${aboveSurface} violaciones`);
check("los túneles de mina no tocan el bedrock", inBedrock === 0, `${inBedrock} violaciones`);
...
function surfaceAt(data, cx, cz, x, z) {
  for (let y = 63; y >= 0; y--) {
...
function pondInArea(pondFn, liquidId) {
  ...
  for (let wx = -R * 16; wx < R * 16; wx++) {
    for (let wz = -R * 16; wz < R * 16; wz++) {
      ...
      if (h < 0) continue;
      return { wx, wz, h, surf: data[IDX(x, h, z)], below: data[IDX(x, h - 1, z)], liquidId };
    }
  }
  return null;
}
  ...
  for (let wx = -R * 16 - 200; wx < R * 16 + 200; wx++) {
    for (let wz = -R * 16 - 200; wz < R * 16 + 200; wz++) {
      ...
      if (h >= 0 && data[IDX(x, h, z)] === B.LAVA) {
        return { wx, wz, h, surf: data[IDX(x, h, z)], below: data[IDX(x, h - 1, z)] };
      }
    }
  }
  return null;
    ...
    for (let z = 0; z < 16; z++) {
      for (let y = 0; y < 64; y++) {
        if (data[IDX(x, y, z)] === B.CHEST) {
          chests++;
          let h = surfaceAt(data, cx, cz, x, z);
          chestY.push([y, h]);
        }
      }
    }
...
check("isPondAt es determinista", world.isPondAt(5, 5) === world.isPondAt(5, 5));
check("isLavaPondAt es determinista", world.isLavaPondAt(5, 5) === world.isLavaPondAt(5, 5));
...
// ============================================================
{
  ...
  check("el archivo de chunk se comprime con gzip (cabecera 1f 8b)", raw[0] === 0x1f && raw[1] === 0x8b);
  const parsed = world.readChunkFile(file, "test");
  check(
    "el chunk gzip se lee y descomprime igual",
    parsed && parsed.cx === 3 && parsed.cz === -2 && parsed.data[1234] === B.CHEST,
  ...
  fs.writeFileSync(plain, JSON.stringify({ schemaVersion: SCHEMA_VERSION, cx: 9, cz: 9, data: Array.from(arr) }));
  ...
  check("el gzip descomprime a JSON válido con schemaVersion", viaGun.schemaVersion === SCHEMA_VERSION);
}

// ============================================================
// 3) LAVA: no sólida y no minable
// ============================================================
check("la lava no es sólida (isSolidBlock(LAVA) === false)", constants.isSolidBlock(B.LAVA) === false);
check("la lava no es minable (NOT_MINEABLE)", constants.NOT_MINEABLE.has(B.LAVA));
