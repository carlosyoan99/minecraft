...
(async () => {
  ...
  const { LOD_ON_DIST, LOD_OFF_DIST, lodTierFor } = await import('file://' + tmp);
  fs.unlinkSync(tmp);

  check('distancia de entrada a LOD > distancia de salida (banda de histéresis)',
    LOD_ON_DIST > LOD_OFF_DIST, `${LOD_OFF_DIST}..${LOD_ON_DIST}`);
  ...
  check('exactamente LOD_ON_DIST desde full → full (no supera)', lodTierFor(LOD_ON_DIST, 'full') === 'full');
  check('LOD_ON_DIST + 0.1 desde full → lod', lodTierFor(LOD_ON_DIST + 0.1, 'full') === 'lod');

  // --- Frontera de salida (lod → full) ---
  check('exactamente LOD_OFF_DIST desde lod → lod (no baja)', lodTierFor(LOD_OFF_DIST, 'lod') === 'lod');
  check('LOD_OFF_DIST - 0.1 desde lod → full', lodTierFor(LOD_OFF_DIST - 0.1, 'lod') === 'full');

  // --- Histéresis: en la banda se conserva el tier actual ---
  const mid = (LOD_ON_DIST + LOD_OFF_DIST) / 2;
  check('en la banda (50% entre ambos) desde full → full', lodTierFor(mid, 'full') === 'full');
  check('en la banda (50% entre ambos) desde lod → lod', lodTierFor(mid, 'lod') === 'lod');
  ...
  for (let d = LOD_ON_DIST; d >= LOD_OFF_DIST; d -= 0.5) {
    ...
    if (next !== tier) { flips++; tier = next; }
  }
  check('barrer la banda entera desde full: 0 flips', flips === 0, `${flips} flip(s)`);
  tier = 'lod'; flips = 0;
  for (let d = LOD_ON_DIST; d >= LOD_OFF_DIST; d -= 0.5) {
    const next = lodTierFor(d, tier);
    if (next !== tier) { flips++; tier = next; }
  }
  check('barrer la banda entera desde lod: 0 flips', flips === 0, `${flips} flip(s)`);
  ...
  check('acercándose a un chunk LOD: 1 flip (lod→full) y nunca vuelve', flips === 1 && tier === 'full', `${flips} flip(s), tier=${tier}`);
  ...
  check('alejándose de un chunk full: 1 flip (full→lod) y nunca vuelve', flips === 1 && tier === 'lod', `${flips} flip(s), tier=${tier}`);
