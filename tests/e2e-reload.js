...
ws.on("message", (d) => {
	...
	if (m.event === "chat" && typeof m.data.message === "string") {
		if (phase === "watch" && m.data.message.includes("Recetas recargadas")) {
			check(
				"watcher: recarga al editar recetas.json (chat de sistema)",
				true,
				m.data.message.slice(0, 60)
			);
			phase = "cmd";
		...
		if (phase === "cmd" && m.data.message.includes("y atlas solicitado")) {
			check(
				"/reload: recarga a petición (chat de sistema)",
				true,
				m.data.message.slice(0, 60)
			);
		...
		if (phase === "bad" && m.data.message.includes("NO recargadas")) {
			check(
				"JSON inválido → rechazo controlado (swap atómico)",
				true,
				m.data.message.slice(0, 60)
			);
			...
			check(
				"textures_reload llegó al cliente (watcher o /reload)",
				texturesReloads >= 1,
