...
function setNight() {
  state.timeOffset =
    (DAY_CYCLE_MS / 2 + 5000 - (Date.now() % DAY_CYCLE_MS) + DAY_CYCLE_MS) % DAY_CYCLE_MS;
}
function setDay() {
...
{
  const grid = [
    { id: 18, count: 1 }, { id: 18, count: 1 }, { id: 18, count: 1 },
    { id: 7, count: 1 }, { id: 7, count: 1 }, { id: 7, count: 1 },
    null, null, null,
  ];
  const r = crafting.matchRecipe(grid);
  check("receta de la cama (3 lana + 3 tablones → 24)", r && r.result.id === 24, r ? r.result.id + "" : "sin receta");
  check("receta NO sale sin el patrón", !crafting.matchRecipe([{ id: 18, count: 1 }, null, null, null, null, null, null, null, null]));
}
...
ctx.ws.emit("message", JSON.stringify({ event: "sleep", data: { x: bx, y: by, z: bz } }));
...
check("dormir de noche → broadcast time_set al amanecer", ctx.ws.events("time_set").length === 1);
const t = ctx.ws.events("time_set")[0];
check("time_set apunta al día (< mitad del ciclo)", t && t.data.dayTime < DAY_CYCLE_MS / 2, t ? t.data.dayTime + "" : "");
check("respawnPoint fijado en la cama (coordenadas del bloque)", !!p.respawnPoint && Math.floor(p.respawnPoint.x) === bx && Math.floor(p.respawnPoint.y) === by && Math.floor(p.respawnPoint.z) === bz, JSON.stringify(p.respawnPoint));

// No es una cama / lejos → ignorado sin respuesta
world.setBlock(bx + 3, by, bz, B.STONE);
setNight();
ctx.ws.sent.length = 0;
ctx.ws.emit("message", JSON.stringify({ event: "sleep", data: { x: bx + 3, y: by, z: bz } }));
check("dormir sobre un no-cama → ignorado", ctx.ws.events("sleep_ok").length === 0 && ctx.ws.events("sleep_rejected").length === 0);
ctx.ws.emit("message", JSON.stringify({ event: "sleep", data: { x: bx + 40, y: by, z: bz } }));
...
// ============================================================
{
  ...
  check("morir con respawnPoint → reaparece sobre la cama", tp && tp.data.x === 42.5 && tp.data.y === 12 && tp.data.z === -8.5, tp ? JSON.stringify(tp.data) : "sin teleport");
...
// ============================================================
{
  ...
  check("romper la cama suelta el ítem de cama", p3.inventory.some((s) => s && s.id === B.BED));
}
