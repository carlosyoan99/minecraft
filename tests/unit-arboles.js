...
const { CHUNK_SIZE, WORLD_HEIGHT, B } = require('../constants.js');
...
try {
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
          ...
          for (let y = 1; y < WORLD_HEIGHT; y++) {
            if (world.getBlock(baseX + x, y, baseZ + z) === B.OAK_LOG) {
              ...
              if (below !== B.OAK_LOG) bases.push({ x: baseX + x, y, z: baseZ + z, below });
            }
          }
        }
      }
  ...
  check('hay columnas de bosque/llanura (condición del test)', forestPlains > 0, `${forestPlains} columnas`);
  check('se generaron troncos en la zona de prueba', bases.length > 0, `${bases.length} bases de tronco`);

  // Invariante principal del fix: ninguna base flota (debajo nunca aire/agua)
  const floating = bases.filter((b) => b.below === B.AIR || b.below === B.WATER);
  check('ningún tronco flota: base sobre bloque sólido', floating.length === 0,
    floating.length > 0 ? `ej. x=${floating[0].x} y=${floating[0].y} z=${floating[0].z} debajo=${floating[0].below}` : `${bases.length} troncos sobre superficie`);

  // Más fuerte: en bosque/llanura la base descansa sobre el césped (height-1)
  const notGrass = bases.filter((b) => b.below !== B.GRASS);
  check('las bases descansan sobre césped (height-1)', notGrass.length === 0,
    notGrass.slice(0, 3).map((b) => `x=${b.x} y=${b.y} debajo=${b.below}`).join('; '));
} finally {
  Math.random = realRandom;
}

console.log(failed === 0 ? `✅ unit-arboles: ${passed} checks OK` : `❌ unit-arboles: ${failed}/${passed + failed} fallaron`);
