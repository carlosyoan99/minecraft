...
const { isSolidBlock, B } = require('../constants.js');
...
let blockAt = null;
require('../world.js').getBlock = (wx, wy, wz) => blockAt(wy);
...
check('mob sobre la superficie del agua se hunde al fondo (y≈3, no se queda en y=5)', surfY < 3.5, 'y=' + surfY);
...
check('mob en el fondo del lago reposa (no sube con agua en la cabeza)', fondoY < 3.5, 'y=' + fondoY);
...
check('mob que cae en un lago aterriza en el fondo (no flota en la superficie)', cayendoY < 3.5 && cayendoY > 2.5, 'y=' + cayendoY);
...
check('mob sobre tierra firme se queda donde está (sin regresión)', tierraY === 5, 'y=' + tierraY);
...
check('mob que cae sobre tierra aterriza en el suelo', caidaTierraY < 6 && caidaTierraY >= 5, 'y=' + caidaTierraY);
...
check('mob atascado en bloque sólido sube hasta liberarse', atascadoY > 5, 'y=' + atascadoY);

// --- Sanidad: el agua no es sólida (base del fix) ---
check('isSolidBlock(WATER) === false (el agua no es sólida)', isSolidBlock(B.WATER) === false);
