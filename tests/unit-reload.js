...
try {
  ...
  check('reloadRecipes ok con archivos válidos', r1.ok === true, JSON.stringify(r1));
  check('contadores coinciden con los archivos originales',
    r1.crafting === Object.keys(origCraft).length && r1.furnace === Object.keys(origFurnace).length,
    `${r1.crafting} crafteo / ${r1.furnace} horno`);

  // --- 2) Swap atómico: modificar una receta se nota al instante ---
  const stickKey = Object.keys(origCraft).find((k) => origCraft[k].result && origCraft[k].result.id === 100);
  const stick = origCraft[stickKey];
  check('se encontró la receta del palo (result.id 100) para modificarla', !!stickKey && !!stick);
  ...
  check('matchRecipe usa la receta recargada (count 5)',
  ...
  check('tras JSON inválido matchRecipe sigue viendo la receta anterior (count 5)',
    m3 && m3.result.id === 100 && m3.result.count === 5, `id/count=${m3 && m3.result.id}/${m3 && m3.result.count}`);
  ...
  check('receta malformada → ok:false (swap atómico)', r4.ok === false, r4.error);
  const m4 = crafting.matchRecipe(grid);
  check('tras receta malformada matchRecipe sigue igual (count 5)',
    m4 && m4.result.id === 100 && m4.result.count === 5, `id/count=${m4 && m4.result.id}/${m4 && m4.result.count}`);
  ...
  check('reloadRecipes recarga también las recetas de horno', r5.ok === true,
    `${r5.crafting} crafteo / ${r5.furnace} horno`);
  // El tick usa recipe.time: al cocinar, requiredTicks refleja el cambio.
  check('isCookable sigue funcionando tras recargar (mineral de carbón)',
