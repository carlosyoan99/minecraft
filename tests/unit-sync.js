...
const server = require(path.join(ROOT, 'constants.js'));
...
function parseObj(name) {
  ...
  if (!m) return null;
  const out = {};
  for (const [, k, v] of m[1].matchAll(/(\d+):\s*(0x[0-9a-fA-F]+|\d+|'[^']*')/g)) out[Number(k)] = v;
  return out;
}
...
// --- 1) Bloques: BLOCK_COLORS del cliente cubre TODOS los IDs de B ---
{
  ...
  check('BLOCK_COLORS cliente cubre todos los bloques de B (salvo el aire)', !!clientBlocks && missing.length === 0,
    'faltan: ' + missing.join(','));
  // Y no hay bloques extra (cliente = servidor, mismo universo)
  const extra = Object.keys(clientBlocks || {}).map(Number).filter((id) => !Object.values(server.B).includes(id));
  check('BLOCK_COLORS no tiene bloques que el servidor no conozca', extra.length === 0, 'extra: ' + extra.join(','));
}
...
// --- 2) Ítems: ITEM_NAMES del cliente cubre todos los IDs de I ---
{
  ...
  check('ITEM_NAMES cliente cubre todos los ítems de I', !!clientItems && missing.length === 0,
    'faltan: ' + missing.join(','));
}
...
// --- 3) Durabilidad: DURABILITY (cliente) == TOOL_DURABILITY (servidor) ---
{
  ...
  check('DURABILITY cliente == TOOL_DURABILITY servidor', equal,
...
// --- 4) Comida: FOOD_ITEMS del cliente == claves de FOOD_VALUES del servidor ---
{
  ...
  check('FOOD_ITEMS cliente == FOOD_VALUES servidor', equal,
    equal ? '' : `cliente=[${[...(clientFood||[])].join(',')}] servidor=[${[...serverFood].join(',')}]`);
}
...
// --- 5) Cría: BREED_FOOD cliente == valores de BREED_FOOD servidor ---
{
  ...
  check('BREED_FOOD cliente == BREED_FOOD servidor', equal,
    equal ? '' : `cliente=[${[...(clientBreed||[])].join(',')}] servidor=[${[...serverBreed].join(',')}]`);
}
...
check('WATER cliente (20) == servidor', parseNum('WATER') === server.B.WATER, 'cliente=' + parseNum('WATER'));
check('SNOW cliente (21) == servidor', parseNum('SNOW') === server.B.SNOW, 'cliente=' + parseNum('SNOW'));
check('XP_PER_LEVEL cliente == servidor', parseNum('XP_PER_LEVEL') === server.XP_PER_LEVEL,
  'cliente=' + parseNum('XP_PER_LEVEL'));
check('DAY_CYCLE_MS cliente == servidor', parseNum('DAY_CYCLE_MS') === server.DAY_CYCLE_MS,
...
// --- 8) Los items de crafteo de la Fase 3/5 están todos en ITEM_NAMES ---
{
  ...
  check('toda la comida tiene nombre en el cliente', foodIds.every((id) => id in clientItems));
  check('los ítems de cría (115-117) tienen nombre', [115, 116, 117].every((id) => id in clientItems));
  check('hilo (120) y conejo (118/119) tienen nombre', [118, 119, 120].every((id) => id in clientItems));
}
