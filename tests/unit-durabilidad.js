...
const {
  TOOL_DURABILITY, SWORD_DAMAGE, isTool, XP_PER_LEVEL, MAX_LEVEL_HEALTH_BONUS,
  MOB_XP, ORE_XP, B, I,
...
function mkPlayer(over = {}) {
  return {
    ...
    health: 20, maxHealth: 20, xp: 0, level: 0,
    food: 20, saturation: 20, foodAccum: 0, regenAccum: 0, starveAccum: 0,
    lastMoveTime: 0, x: 0, y: 64, z: 0,
...
// 1) TOOL_DURABILITY cubre todas las herramientas (200-219)
{
  ...
  check('diamante dura más que madera (progresión)', TOOL_DURABILITY[204] > TOOL_DURABILITY[200]);
}
...
// 2) addToInventory: las herramientas llevan durabilidad plena y NO se apilan
{
  ...
  check('herramienta nueva con durabilidad plena', slots[0].durability === TOOL_DURABILITY[I.WOODEN_PICKAXE],
...
// 3) applyToolWear: -1 por uso, y se rompe al llegar a 0
{
  ...
  check('wear 1: -1 durabilidad', playersMod.applyToolWear(p) === false && p.inventory[0].durability === 1);
  check('wear 2: se rompe y elimina el slot', playersMod.applyToolWear(p) === true && p.inventory[0] === null);
  check('wear 3: mano vacía no desgasta', playersMod.applyToolWear(p) === false);
}
...
// 4) onlySwords: al atacar solo se desgasta la espada
{
  ...
  check('atacar con espada desgasta', playersMod.applyToolWear(p, true) === false && p.inventory[0].durability === 2);
  const q = mkPlayer({ selectedSlot: 0 });
  q.inventory[0] = { id: I.IRON_PICKAXE, count: 1, durability: 5 };
  check('atacar con pico NO desgasta (solo espadas)', playersMod.applyToolWear(q, true) === false && q.inventory[0].durability === 5);
}
...
//    (secuencia del handler break: se añade el drop y luego se desgasta/elimina la herramienta)
{
  ...
  check('el drop NO se duplica (1 adoquín)', playersMod.countInInventory(p, B.COBBLESTONE) === 1,
    'count=' + playersMod.countInInventory(p, B.COBBLESTONE));
  check('la herramienta rota desaparece', playersMod.countInInventory(p, I.STONE_PICKAXE) === 0);
}
...
//    modo que NO se "repara gratis" una herramienta dañada (evita duplicar usos)
{
  ...
  check('round-trip por la mesa conserva durabilidad (no se repara gratis)',
    found.length === 1 && found[0].durability === 30, 'dur=' + (found[0] && found[0].durability));
}
...
// ============================================================
console.log('\n=== DAÑO DE ESPADA ===');
check('todas las espadas tienen daño', [215, 216, 217, 218, 219].every((id) => SWORD_DAMAGE[id] > 0));
check('diamante > hierro > piedra > madera', SWORD_DAMAGE[219] > SWORD_DAMAGE[217] && SWORD_DAMAGE[217] > SWORD_DAMAGE[216] && SWORD_DAMAGE[216] > SWORD_DAMAGE[215]);
check('oro más débil que hierro (como Minecraft)', SWORD_DAMAGE[218] < SWORD_DAMAGE[217]);
...
// 8) Tope de salud máxima: +10 como mucho (nivel 10+)
{
  ...
  check('maxHealth tope en 30 (+10)', p.maxHealth === 20 + MAX_LEVEL_HEALTH_BONUS, 'max=' + p.maxHealth);
}
...
check('ORE_XP cubre los 6 minerales', [9, 10, 11, 12, 13, 14].every((id) => ORE_XP[id] > 0));
check('MOB_XP cubre hostiles y pasivos nuevos', MOB_XP.spider > 0 && MOB_XP.wolf > 0 && MOB_XP.rabbit > 0 && MOB_XP.zombie > 0);
...
// 11) Salud por tipo
check('spider 12 HP (frágil)', new mobsMod.Mob('spider', 0, 64, 0).health === 12);
check('wolf 20 HP', new mobsMod.Mob('wolf', 0, 64, 0).health === 20);
check('rabbit 10 HP (pasivo)', new mobsMod.Mob('rabbit', 0, 64, 0).health === 10);
...
// 12) Drops: la araña suelta hilo (118), el conejo carne cruda (118→hornear)
{
  ...
  check('spider dropea hilo (118)', spider && spider[0] && spider[0].id === I.STRING, JSON.stringify(spider));
  check('rabbit dropea conejo crudo (118)', rabbit && rabbit[0] && rabbit[0].id === I.RABBIT, JSON.stringify(rabbit));
}

// 13) El conejo se cría con zanahorias y el hilo sirve para lana (recetas)
check('rabbit se alimenta con zanahoria (cría)', mobsMod.canFeed({ type: 'rabbit', isBaby: false, cooldownUntil: 0 }, I.CARROT) === 'ok');
{
  const grid = new Array(9).fill(null);
  for (const idx of [0, 1, 3, 4]) grid[idx] = { id: I.STRING, count: 1 }; // 2x2 hilo
  const r = crafting.matchRecipe(grid);
  check('2x2 hilo → lana (18)', r && r.result.id === 18, JSON.stringify(r && r.result));
}
...
check('mobSnapshot incluye type (escala cliente)', mobsMod.mobSnapshot({ id: 'x', type: 'spider', color: 1, state: 'idle', isBaby: false }).type === 'spider');
