...
| **Trabajo en curso** | **Fase 21.5 en curso** (abierta 2026-08-17). Implementados y auditados: **D2/D3** (diferidos de la F21: océanos cálidos con corales y montañas altas — spec F21.5 §1.4), **A1/A8** (pesca, caña + bobber + tabla de loot + receta; cañas rotas en cofres), **B1** (piedra pulida: granito/diorita/andesita + pulidas, vetas hash 2D), **B2** (linternas, luz de antorcha), **B3** (bambú/andamios: planta alta estática 3-8, tablones 2×2, andamio no sólido escalable que sube al colocarlo bajo los pies), **B4** (colmenas/miel: nido en árboles, colmena crafteada, botella de vidrio → botella de miel 6/1.2, bloque de miel anula caída, recetas), **B5** (coral y algas: abanico sobre arrecife, kelp 2-6 y pasto marino en el océano). Suite `--unit` completa en verde (incl. `unit-fase21.5` con 13 secciones). |
...
| **Bloqueantes** | Ninguno |
| **Próximo paso** | Continuar con el siguiente bloque de la F21.5 (B5 coral/algas pendiente de pulir, C2 escudo, C3 tótem, C4 camas de colores, E1/E2 animales y lana, G1 comandos — ver `TODO.md`); cerrar la fase con auditoría y commit |
