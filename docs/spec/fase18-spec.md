...
| **CSP + SRI del CDN de Three.js** (SEC-4, auditoría 2026-08-11) | **Resuelto**: Three.js se sirve **local** (`public/vendor/three.module.js` + addons, importmap propio) desde la Fase 19.6 — ya no hay CDN externo que asegurar con CSP/SRI. El importmap es un `<script type="importmap">` inline (exige `script-src 'unsafe-inline'`); el juego es localhost/LAN sin autenticación. Aplicado: `X-Content-Type-Options: nosniff` en el estático (`server/net.js`) y service worker opcional (`public/sw.js`). | Cerrado |
