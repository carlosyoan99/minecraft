...
function materialFor(blockId) {
  if (blockId === 3 || blockId === 8 || blockId === 16 || blockId === 19 || (blockId >= 9 && blockId <= 14)) return MATERIALS.stone;
...
export function playStep(blockId) {
  ...
  const t = ctx.currentTime + 0.001;
  stepAlt = !stepAlt;
  noiseBurst({ t, freq: m.hit * (stepAlt ? 0.9 : 1.1), q: 1.4, vol: 0.12, dur: 0.055, type: 'lowpass' });
}
