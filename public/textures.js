...
const PAL = {
  dirt: '#8a5a2b', dirtDark: '#6f4a22', dirtLight: '#9c6b3a',
  grass: '#5fb83f', grassDark: '#4a9a33', grassLight: '#7ad04f',
  stone: '#8a8a8a', stoneDark: '#6f6f6f', stoneLight: '#a0a0a0',
  bark: '#6b4a2b', barkDark: '#4f3520', barkLight: '#8a6840',
  wood: '#c9a46b', woodDark: '#a8824f', woodLight: '#d9b884',
  leaf: '#3a7a2e', leafDark: '#2c5f22', leafLight: '#4a9440',
  sand: '#e0c88a', sandDark: '#c9b070', sandLight: '#ecd9a4',
  coal: '#33393d', iron: '#b08968', gold: '#e8c547',
...
function drawGrassTop(ctx, rng) {
  ...
  speckle(ctx, rng, PAL.grassLight, 0.10);
}
function drawGrassSide(ctx, rng) {
  ...
  for (let x = 0; x < TILE; x++) {
    ...
    for (let y = 0; y <= h; y++) px(ctx, x, y, y === 0 ? PAL.grassLight : PAL.grass);
...
function drawStone(ctx, rng) {
  ...
  speckle(ctx, rng, PAL.stoneLight, 0.10);
}
...
function makeOre(oreColor) {
  return (ctx, rng) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      rect(ctx, x, y, 1 + Math.floor(rng() * 3), 1 + Math.floor(rng() * 3), oreColor);
    }
  };
}
...
function drawFurnaceFront(ctx, rng) {
  ...
  speckle(ctx, rng, PAL.furnaceDark, 0.10);
...
export function tileForFace(blockId, faceIndex) {
  ...
  if (t.fronts !== undefined && (faceIndex === 4 || faceIndex === 5)) return t.fronts; // ±Z
  return t.side;
}
