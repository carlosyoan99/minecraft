...
function floodInto(out, chunkSize, worldHeight, x0, z0, tx, ty, tz, blockAt) {
  ...
  while (head < queue.length) {
    const i = queue[head++];
    const l = scratch[i];
    const lx = i % S, ly = (Math.floor(i / S)) % S, lz = Math.floor(i / (S * S));
    const wx = tx + lx - LIGHT_RADIUS, wy = ty + ly - LIGHT_RADIUS, wz = tz + lz - LIGHT_RADIUS;
    const nl = l * LIGHT_ATTEN;
    if (nl < LIGHT_MIN) continue;
    for (const [dx, dy, dz] of DIRS) {
      const nlx = lx + dx, nly = ly + dy, nlz = lz + dz;
      if (nlx < 0 || nlx >= S || nly < 0 || nly >= S || nlz < 0 || nlz >= S) continue;
  ...
  const minX = Math.max(tx - LIGHT_RADIUS, x0), maxX = Math.min(tx + LIGHT_RADIUS, x0 + chunkSize - 1);
  const minZ = Math.max(tz - LIGHT_RADIUS, z0), maxZ = Math.min(tz + LIGHT_RADIUS, z0 + chunkSize - 1);
  for (let wz = minZ; wz <= maxZ; wz++) {
    const lz = wz - (tz - LIGHT_RADIUS);
    for (let wy = Math.max(0, ty - LIGHT_RADIUS); wy <= Math.min(worldHeight - 1, ty + LIGHT_RADIUS); wy++) {
...
export function computeChunkLight(cx, cz, chunkSize, worldHeight, blockAt, torches) {
  ...
  for (const t of torches) {
    ...
    if (tx < x0 - LIGHT_RADIUS || tx > x0 + chunkSize - 1 + LIGHT_RADIUS) continue;
    if (tz < z0 - LIGHT_RADIUS || tz > z0 + chunkSize - 1 + LIGHT_RADIUS) continue;
